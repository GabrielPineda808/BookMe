...
public class ServiceManagementService {
    ...
    public ServiceManagementService(ServiceRepository serviceRepository, UserRepository userRepository,
    ...
    public com.example.book.model.Service updateService(Long serviceId, ServiceDto input, String email) {
        ...
        if (!affectedBookings.isEmpty()) {
            // Handle affected bookings - you might want to notify users or create change
            // requests
    ...
    public void deleteService(Long serviceId, String email, boolean forceDelete) {
        ...
        if (activeBookingsCount > 0 && !forceDelete) {
            throw new ActiveBookingsException(
                    "Cannot delete service with active bookings. Use force delete to proceed, but this will cancel all bookings.");
        }

        if (activeBookingsCount > 0 && forceDelete) {
            // Cancel all active bookings
            List<Booking> activeBookings = bookingRepository.findByServiceIdAndStatusIn(serviceId,
    ...
    private List<Booking> checkBookingImpact(com.example.book.model.Service service, LocalTime newOpen,
            LocalTime newClose, int newInterval) {
    ...
    private void handleAffectedBookings(com.example.book.model.Service service, List<Booking> affectedBookings) {
        ...
        // i need to decide whether i want to keep old booking times or not and not let
        // them change as they were made previously
        // either automatic chagne request or if it does not affect next or previous
        // booking change it to the new service times
